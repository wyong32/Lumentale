...
export function buildArticleJsonLd({ headline, description, url, datePublished }) {
  const img = toAbsoluteUrl(seoConfig.defaultOgImage)
  const siteUrl = seoConfig.fullDomain.replace(/\/+$/, '')
  return {
    '@context': 'https://schema.org',
    '@type': 'Article',
...
export function buildGraphJsonLd(...nodes) {
  return {
    '@context': 'https://schema.org',
    '@graph': nodes,
  }
